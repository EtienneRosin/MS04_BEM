...
class BSPNode:
    ...
    def __init__(
        ...
        new_indexing: np.ndarray,
        # new_indexing: np.ndarray,
        global_indexing: np.ndarray,
        depth: int = 0,
        parent=None,
        largest_dimension_axis = None,
        ...
        - new_indexing: np.ndarray -> Global indices of points for external reference.
        ...
        self.new_indexing = new_indexing
        ...
        self.largest_dimension_axis = np.argmax(self.extents) if largest_dimension_axis is None else largest_dimension_axis
    ...
    def split(self):
        ...
        self.childrens = [
            BSPNode(
                ...
                new_indexing=self.new_indexing[left_indices],
                depth=self.depth + 1,
                parent=self,
                largest_dimension_axis = not(self.largest_dimension_axis),
                global_indexing=self.global_indexing[:N]
            ),
            BSPNode(
                ...
                new_indexing=self.new_indexing[right_indices],
                depth=self.depth + 1,
                parent=self,
                largest_dimension_axis = not(self.largest_dimension_axis),
                global_indexing=self.global_indexing[N:]
...
class BSPTree:
    ...
    def __init__(self, points: np.ndarray) -> None:
        ...
        self.root = BSPNode(
            points = points, 
            indices_x_sorted=self.indices_x_sorted, 
            indices_y_sorted=self.indices_y_sorted, 
            new_indexing = np.arange(points.shape[0]),
            global_indexing = np.arange(points.shape[0])
            )
        self.max_depth = 0
        self.indexing = None
    ...
    def display(self, cmap = cmr.lavender, alpha = 0.75):
